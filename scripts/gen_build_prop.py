...
def get_build_keys(product_config):
  ...
  if default_cert == "" or default_cert == os.path.join(TEST_KEY_DIR, "testKey"):
    return "test-keys"
  if default_cert.startswith("vendor/lmodroid-priv/"):
    return "release-keys"
  return "dev-keys"
...
def parse_args():
  ...
  config["LMODroidDesc"] = config["BuildDesc"]
  config["LMODroidDevice"] = config["DeviceName"]
...
def generate_build_info(args):
  ...
  if config["BuildVariant"] == "user":
    ...
    if config["LMODroidBuildType"] == "UNOFFICIAL":
    ...
    print(f"ro.build.display.id?={config['LMODroidDesc']}")
  ...
  print(f"ro.lmodroid.device={config['LMODroidDevice']}")
